...
//
// IMPORTANTE (revertido — ver caso real de folios 654/655, CEDIS
// Villahermosa, mismo T.U. en ambos): este correo NUNCA debe buscar ni
// corregir una autorización ya CONFIRMADA por T.U. — el T.U. se repite
// legítimamente entre renglones DISTINTOS (dos tramos, o un renglón
// duplicado en el correo del cliente donde uno trae el monto y otro no).
// El identificador que de verdad distingue un renglón de otro es el
// FOLIO/PEDIDO, nunca el T.U. — por eso este correo solo cierra una
// autorización que esté "pendiente" (esperando monto, sin resolver
// todavía); si no hay ninguna pendiente que coincida, SIEMPRE se registra
// un renglón nuevo en $0 (nunca se toca una ya confirmada por más que
// comparta T.U. con ella).
//
// Si no hay ninguna Pendiente (caso más común: el "NO PAGA" es la ÚNICA
...
// identificar al operador (no se puede registrar un ingreso sin unidad);
// si no se encuentra, se deja sin resolver (poco común, ya que casi
// siempre trae el económico).
...
async function _compactarPdfCartaPorteServer(pdfOriginalBuffer, cartaPorteData, fac) {
  ...
  // dibujarParrafo: ajusta por palabra según el ancho real de la fuente
  // (a diferencia de campoTextoLargo, que corta a lo bruto por ser texto
  // sin espacios) — para las cláusulas del contrato, que son prosa legal.
  function dibujarParrafo(texto, size) {
    size = size || 8;
    ...
    palabras.forEach(function (palabra) {
      ...
      if (font.widthOfTextAtSize(propuesta, size) > anchoDisponible && linea) {
        nuevaPaginaSiHaceFalta();
        pagina.drawText(linea, { x: margin, y, size: size, font: font });
        y -= size + 3;
        linea = palabra;
      } else {
        linea = propuesta;
...
// de Servicios que ampara la Carta de Porte o Comprobante para Amparar el
// Transporte de Mercancías", publicado por la SCT en el Diario Oficial de
// la Federación el 15 de diciembre de 2015 (Circular sobre modelos de
// Carta de Porte) — se transcriben tal cual, sin modificar el contenido
// (el punto SÉPTIMO de esa misma Circular prohíbe variarlo).
const CLAUSULAS_SCT_CARTA_PORTE = [
  ...
  'SÉPTIMA.- Si la carga no fuere retirada dentro de los 30 días hábiles siguientes a aquél en que hubiere sido puesta a disposición del consignatario, el "Transportista" podrá solicitar la venta en subasta pública con arreglo a lo dispone el Código de Comercio.',
...
// wialonActualizarBitacorasProgramado (corría sola cada 15 min) se eliminó
// por completo — costaba el trabajo de Cloud Scheduler aunque no hiciera
// nada (ese costo es por trabajo programado que existe, no por qué tan
// seguido corre) y los operadores todavía no usan operador.html para que
// haga falta. El registro manual de bitácora con botones (como al inicio)
// sigue disponible vía wialonActualizarBitacoras (arriba), sin trigger
// automático.
